...
@Component({
    ...
    imports: [CommuniqueList, PaginatorModule, AppFilterComponent],
...
export class CommuniquePage implements OnInit {
  ...
  private translateService: TranslateService = inject(TranslateService);
    ...
    fetchCommuniques(filter: CommuniqueFilterClass = this.currentFilter, page: number = this.currentPage, size: number = this.rows) {
        ...
        this.communiqueService.getFilteredCommunique(this.currentFilter, this.currentPage, this.rows).subscribe({
            next: (data: any) => {
